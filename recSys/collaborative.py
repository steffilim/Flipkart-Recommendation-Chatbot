...
def svd_recommend_surprise(user_id, keywords, orderdata, n_recommendations=20):
    ...
    filtered_products = flipkart[flipkart['uniq_id'].isin(recommended_product_ids)]

    keyword_filter = filtered_products['product_category_tree'].apply(lambda cat_tree: any(kw.lower() in str(cat_tree).lower() for kw in keywords))
    
    final_recommendations = filtered_products[keyword_filter].head(n_recommendations)
    
    return final_recommendations[['uniq_id', 'product_name', 'description', 'product_category_tree']]


# To test
# user_id = 'U06610'
# keywords = ['clothing', 'car']
# recommendations = svd_recommend_surprise(user_id, keywords, orderdata)
