...
def svd_recommend_surprise(catalogue, user_id, orderdata, n_recommendations=20):
    ...
    recommendations = list(catalogue.find({'uniq_id': {'$in': recommended_product_ids}}))
    for rec in recommendations:
        rec['predicted_rating'] = next((pred[1] for pred in predictions if pred[0] == rec['uniq_id']), None)
    recommendations_df = pd.DataFrame(recommendations)
    if not recommendations_df.empty:
        recommendations_df = recommendations_df[['uniq_id', 'product_name', 'description', 'product_category_tree', 'predicted_rating', 'retail_price']]
